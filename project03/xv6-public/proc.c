...
int thread_create(thread_t *thread, void *(*start_routine)(void *), void *arg){
	...
	//reference from fork()
	if((np = allocproc()) == 0){ //allocate thread
		return -1;
	}
	nextpid--; //since allocated process was a thread

	//analyze the master thread
        if(curproc->master == 0){
                master = curproc;
        }
        else{
	...
	if(master->pgdir == 0){ //exection handling if master thread is invalid
		np->state = UNUSED;
		return -1;
	}

	master->sz = PGROUNDUP(master->sz);
	if ((master->sz = allocuvm(master->pgdir, master->sz, master->sz + 2 * PGSIZE)) == 0) {
		return -1;
	}
	clearpteu(master->pgdir, (char*)(master->sz - 2 * PGSIZE));
	sp = master->sz;
